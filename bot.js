...
client.on("guildMemberRemove", async member => {
 ...
 .setColor("AEDD13")
  ...
  try {
    ...
    giriscikiskanali.send(`<a:outbox_tray:638330859774017546> | \`${member.user.tag}\`, aramızdan ayrıldı, \`${sayac[member.guild.id].sayi}\` kişi olmamıza \`${sayac[member.guild.id].sayi - member.guild.memberCount}\` kişi kaldı!`);
...
client.on("guildMemberAdd", async member => {
 ...
 .setColor("AEDD13")
  ...
  try {
    let giriscikiskanalID = giriscikis[member.guild.id].kanal;
    let giriscikiskanali = client.guilds.get(member.guild.id).channels.get(giriscikiskanalID);
    giriscikiskanali.send(`<a:inbox_tray:638756375975034907> | \`${member.user.tag}\`, sunucuya katıldı \`${sayac[member.guild.id].sayi}\` kişi olmamıza \`${sayac[member.guild.id].sayi - member.guild.memberCount}\` kişi kaldı!` );
  } catch (e) { // eğer hata olursa bu hatayı öğrenmek için hatayı konsola gönderelim.
    return console.log(e)
  }
...
client.on("guildMemberAdd", async member => {
  ...
  let embed = new Discord.RichEmbed()
    ...
    .setDescription(`<a:ok_hand:638702179623895050> | Sunucuya hoşgeldin \`@${member.user.tag}\` otomatik rol verildi. `)
.setColor("GREEN")
  ...
  try {
    let giriscikiskanalID = giriscikis[member.guild.id].kanal;
    let giriscikiskanali = client.guilds.get(member.guild.id).channels.get(giriscikiskanalID);
    giriscikiskanali.send(`<a:ok_hand:638702179623895050> | Sunucuya hoşgeldin \`@${member.user.tag}\` otomatik rol verildi.`);
  } catch (e) { // eğer hata olursa bu hatayı öğrenmek için hatayı konsola gönderelim.
    return console.log(e)
  }
...
client.on("message", async message =>{
  ...
  let i = await   db.fetch(`saas_${message.guild.id}`)
       if(i === "acik"){
         message.channel.send(`**Aleyküm Selam, hoşgeldin.** <a:hugging:641679684526145592>`)
...
client.on("guildCreate", guild => {
  const tesekkurler = new Discord.RichEmbed()
    ...
    .setTimestamp()
    .setFooter("Developed by Deimos")
    .setColor("GREEN")
    .setDescription("Beni Sunucuna Eklediğin İçin Teşekkür Ederim \n Sana En İyi Şekilde Hizmet Edeceğim.\n Eğer Bir Sorunla Karşılaşırsan Destek Sunucuma Gel [Destek Sunucum](https://discord.gg/S45VxGv)")
